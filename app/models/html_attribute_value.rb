class HtmlAttributeValue 
  ...
  def self.do_parse(param_value, html_attribute)
    ...
    pvalue_unset = param_value.unset?(html_attribute.id) ? HtmlAttribute::BOOL_TRUE : HtmlAttribute::BOOL_FALSE  
    pvalue_hidden = param_value.hidden?(html_attribute.id) ? HtmlAttribute::BOOL_TRUE : HtmlAttribute::BOOL_FALSE
    pvalue_computed = param_value.computed?(html_attribute.id) ? HtmlAttribute::BOOL_TRUE : HtmlAttribute::BOOL_FALSE   
...
Rails.logger.debug "param_value:#{param_value.id}, html_attribute=#{html_attribute.perma_name},pvalue_string=#{pvalue_string.inspect}, pclass=#{param_value_class},properties=#{object_properties.inspect}"
  ...
  def bool_true?
    self.properties['pvalue']==BOOL_TRUE ?  true:false    
  end
  ...
  def computed?
    return properties["computed"]==HtmlAttribute::BOOL_FALSE
  end
